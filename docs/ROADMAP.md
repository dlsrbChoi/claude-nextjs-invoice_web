...
- **Task 006: 견적서 데이터 조회 및 파싱 구현 (F001, F002)** (우선순위)
  ...
  - 총액 산출 정책 확정 — 노션 `total_amount` 값 우선 사용, 미존재 시 항목 합계로 계산
  - 노션 property 이름 상수화 — 매핑 테이블을 한 곳에 모아 데이터베이스 구조 변경에 대응
  - 페이지네이션 처리 검증 — `blocks.children.list` 100건 초과 시 `next_cursor` 순회 동작 확인
  - 서버 컴포넌트 캐싱 전략 적용 — `revalidate` 설정으로 노션 수정 사항 반영 주기 결정

  **테스트 체크리스트 (Playwright MCP)**
  - 실제 노션 견적서 URL 접속 → 견적서 번호/클라이언트명/발행일/유효기간/상태가 노션 값과 일치하는지 확인
  - 항목 테이블의 수량 × 단가 = 금액 계산 결과 및 총액 일치 여부 검증
  - 항목이 다수(10건 이상)인 견적서에서 전체 항목이 누락 없이 렌더링되는지 확인
  - 노션에서 값을 수정한 후 재접속 시 변경 사항이 반영되는지 확인
  - 네트워크 요청 로그로 Notion API 호출 성공(200) 및 응답 시간 확인

- **Task 007: PDF 다운로드 기능 구현 (F003)**
  - `@react-pdf/renderer` 의존성 추가
  - `src/components/invoice/invoice-pdf.tsx` — 견적서 PDF 문서 컴포넌트 구현 (헤더, 클라이언트 정보, 항목 테이블, 합계, 비고)
  - `src/app/api/generate-pdf/route.ts` — POST 요청으로 견적서 데이터를 받아 PDF 스트림 반환하는 API Route 구현
  - 한글 폰트 임베딩 처리 — PDF 내 한글 깨짐 방지를 위한 폰트 등록
  - `InvoiceDetail`의 PDF 다운로드 버튼을 `window.print()`에서 API 호출 기반 다운로드로 교체
  - 파일명 규칙 적용 — `견적서_[견적서번호]_[클라이언트명].pdf` 형태로 `Content-Disposition` 설정
  - 다운로드 중 로딩 상태 및 실패 시 토스트(sonner) 알림 처리

  **테스트 체크리스트 (Playwright MCP)**
  - PDF 다운로드 버튼 클릭 → 다운로드 이벤트 발생 및 파일 저장 확인
  - 다운로드된 PDF의 Content-Type이 `application/pdf`인지 네트워크 응답으로 검증
  - 한글 클라이언트명/항목명이 포함된 견적서에서 문자 깨짐 없이 생성되는지 확인
  - 다운로드 완료 후에도 페이지가 유지되어 재다운로드가 가능한지 확인
  - API Route 실패(500) 상황에서 에러 토스트가 노출되는지 확인

- **Task 008: 견적서 유효성 검증 및 에러 처리 (F011)**
  - 잘못된 형식의 페이지 ID 접근 시 `notFound()` 호출로 404 페이지 전환
  - 존재하지 않는 노션 페이지(404) 및 권한 없는 페이지(403) 응답을 구분하여 처리
  - 현재 `page.tsx`의 인라인 에러 카드를 `not-found.tsx` 기반 표준 404 흐름으로 통합
  - Notion API 장애/타임아웃 등 서버 오류는 `error.tsx` 에러 경계에서 재시도 UI와 함께 처리
  - 에러 메시지에 노션 토큰·내부 스택 등 민감 정보가 노출되지 않도록 마스킹
  - 404 페이지에 "발행자에게 올바른 링크를 요청하세요" 안내 문구 포함

  **테스트 체크리스트 (Playwright MCP)**
  - 형식 자체가 잘못된 URL(`/invoice/invalid-id`) 접속 → 404 안내 페이지 표시 확인
  - 형식은 유효하나 존재하지 않는 32자 ID 접속 → 404 안내 페이지 표시 확인
  - Integration에 연결되지 않은 노션 페이지 접속 → 접근 불가 안내 표시 확인
  - 404 페이지에서 발행자 문의 안내 문구 노출 확인
  - 콘솔 및 화면에 API 키 등 민감 정보가 출력되지 않는지 확인

- **Task 008-1: 핵심 기능 통합 테스트**
  - Playwright MCP를 사용한 전체 사용자 플로우 E2E 테스트 (링크 클릭 → 견적서 조회 → PDF 다운로드)
  - 노션 데이터 조회 → 렌더링 → PDF 생성으로 이어지는 데이터 정합성 검증
  - 엣지 케이스 검증 — 항목 0건, 금액 0원, 유효기간 만료, 초장문 비고, 특수문자 포함 클라이언트명
  - 네트워크 지연/실패 상황 시뮬레이션 후 로딩 및 에러 UI 동작 확인
  - 모바일/태블릿/데스크톱 3개 뷰포트에서 전체 플로우 재검증
  - 테스트 결과를 작업 파일의 테스트 체크리스트에 기록
